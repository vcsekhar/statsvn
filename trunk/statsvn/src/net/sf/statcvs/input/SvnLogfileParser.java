...
public class SvnLogfileParser {
    ...
    private void verifyAddition() {
        ...
        for (Iterator iter = fileBuilders.iterator(); iter.hasNext();) {
            ...
            // file's first revision is not a creation... either not in log or was implicit
            if (!fileBuilder.existRevision() || !fileBuilder.getFirstRevision().isCreation()) {
                ...
                if (parentPath.length() > 0)
                    parentPath = parentPath.substring(0, parentPath.length() - 1); // remove last /
                while (parentPath.length() > 0) {
                    ...
                    if (parentPath.length() == 0) {
                        // System.out.println("Did not find creation for " + fileBuilder.getName());
                        break;
                    }
                    parentPath = parentPath.substring(0, parentPath.length() - 1); // remove last /
                }
        ...
        for (Iterator iter = directories.iterator(); iter.hasNext();) {
            ...
            for (Iterator iterator = fileBuilders.iterator(); iterator.hasNext();) {
                ...
                if (child.getName().indexOf(directory + "/") == 0) {
                    // check to see that the addition wasn't somewhere in fileBuilder's existence (meaning we copied over the file from another branch, using
                    // the parent folder)
                    int childRevision;
                    try {
                        int i;
