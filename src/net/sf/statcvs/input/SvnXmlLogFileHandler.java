...
public class SvnXmlLogFileHandler extends DefaultHandler {
	...
	private void endLogEntry() throws SAXException {
		...
		for (int i = 0; i < currentFilenames.size(); i++) {
			...
			boolean isBinary = SvnPropgetUtils.getBinaryFiles().contains(
					currentFilename);
			// is this a deletion?
			if (revisionData.isDeletion()) {
				FileBuilder existingBuilder = (FileBuilder) builder
						.getFileBuilders().get(currentFilename);
				...
				if (existingBuilder != null && !existingBuilder.existRevision())
					...
					isBinary = SvnPropgetUtils.isBinaryFile(revisionData
							.getRevisionNumber(), currentFilename);
			}
			builder.buildFile(currentFilename, isBinary, revisionData
					.isDeletion(), new HashMap());
	...
	public void startElement(String uri, String localName, String qName,
		...
		} else if (eName.equals(AUTHOR) || eName.equals(DATE)
				|| eName.equals(MSG)) {
			startAuthorDateMsg();
		} else if (eName.equals(PATHS)) {
			startPaths();
	...
	private void startLogEntry(Attributes attributes) throws SAXException {
		...
		if (attributes != null && attributes.getValue("revision") != null)
			currentRevisionData.setRevisionNumber(attributes
					.getValue("revision"));
		else
			fatalError("Invalid SVN log file.");
	}
