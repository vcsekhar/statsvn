...
public class CommitListBuilder {
    ...
    /**
     * Creates a new instance using the given set of {@link Revision}s. The set must be sorted by date, oldest first.
     * 
     * @param revisions
    ...
    /**
     * Creates a <code>List</code> of {@link Commit}s from the source iterator. The result list will be sorted by date.
     * 
    ...
    /**
     * Returns <code>true</code> if change is part of the commit, that is if they have the same author, the same message, and are within the same timeframe.
     * 
     * @param commit
     *            the commit
     * @param rev
    ...
    /**
     * Returns <code>true</code> if the date lies within the timespan of the commit, plus/minus a tolerance.
     * 
     * @param date
